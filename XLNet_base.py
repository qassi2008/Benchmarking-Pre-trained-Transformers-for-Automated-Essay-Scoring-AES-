...
from transformers import XLNetTokenizer, XLNetModel, AutoModel, get_cosine_schedule_with_warmup
...
# ====================================================
class CFG:
    model = "xlnet-base-cased"  # Using XLNet-base
    batch_size = 8
    epochs = 100
    encoder_lr = 2e-5
    decoder_lr = 2e-5
...
def MCRMSE(y_true, y_pred):
    ...
    scores = [mean_squared_error(y_true[:, i], y_pred[:, i]) ** 0.5 for i in range(y_true.shape[1])]
...
class FeedbackDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.tokenizer.encode_plus(
            ...
            truncation=True,
            return_tensors="pt",
            return_token_type_ids=True  # Required for XLNet
        )
...
def train_model(train_df, val_df, tokenizer, config):
    ...
    for epoch in range(config.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), OUTPUT_DIR / "best_model_xlnet.pth")
...
# Tokenizer Setup
# ====================================================
tokenizer = XLNetTokenizer.from_pretrained(CFG.model)
...
print("\n=== Training Complete for XLNet-base  ===")
print(f"Best Validation Loss: {best_loss:.4f}")
print(f"Final MCRMSE: {MCRMSE(final_labels, final_preds):.4f}")

# Display sample predictions vs actual values
sample_size = 10
comparison_df = pd.DataFrame(np.hstack([final_preds, final_labels]), columns=CFG.target_cols + [f"actual_{col}" for col in CFG.target_cols])
